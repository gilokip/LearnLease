...
export default function Overview() {
  ...
  if (isLoading) return (
    <div className="space-y-6 animate-fade-up">
      ...
      <div className="grid grid-cols-4 gap-4">
        {[...Array(4)].map((_,i) => <div key={i} className="h-32 rounded-xl animate-pulse" style={{ background: "var(--bg-card)" }} />)}
      </div>
    </div>
  );
  ...
  return (
    <div className="space-y-5 animate-fade-up">
      ...
      {isStudent && studentData && (
        <div className="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-4 gap-4">
          <StatCard icon={<IconCheckCircle size={17} />} title="Lease Status" color="#22c55e"
            ...
            subtitle={studentData.activeLease ? `${studentData.activeLease.brand} ${studentData.activeLease.model}` : "No active lease"} />
          <StatCard icon={<IconCreditCard size={17} />} title="Next Payment" color="#3b5bfc"
            ...
            subtitle={studentData.nextPayment ? `Due ${formatDate(studentData.nextPayment.due_date)}` : "All clear"} />
          ...
          {/* 🛠️ FIXED THE undefined.length BUG HERE 🛠️ */}
          <StatCard icon={<IconClipboard size={17} />} title="Applications" color="#a78bfa"
            value={studentData.leaseHistory?.length ?? 0} subtitle="Total submitted" />
        </div>
      )}
      ...
      {isAdmin && adminData && (
        <div className="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-4 gap-4">
          <StatCard icon={<IconClipboard size={17} />} title="Pending Applications" color="#f59e0b" value={adminData.leases?.pending ?? 0} subtitle="Awaiting review" />
          <StatCard icon={<IconCheckCircle size={17} />} title="Active Leases" color="#22c55e" value={adminData.leases?.active ?? 0} subtitle="All campuses" />
          <StatCard icon={<IconGradCap size={17} />} title="Students" color="#3b5bfc" value={adminData.users?.student ?? 0} subtitle="Registered" />
          <StatCard icon={<IconDollarSign size={17} />} title="Monthly Revenue" color="#a78bfa" value={formatCurrency(adminData.payments?.monthly ?? 0)} subtitle="This month" />
        </div>
      )}
      ...
      {isInventory && adminData && (
        <div className="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-4 gap-4">
          <StatCard icon={<IconLaptop size={17} />} title="Total Devices" color="#3b5bfc" value={adminData.devices?.total ?? 0} subtitle="All models" />
          <StatCard icon={<IconCheckCircle size={17} />} title="Available" color="#22c55e" value={adminData.devices?.available ?? 0} subtitle="Ready to assign" />
          <StatCard icon={<IconActivity size={17} />} title="On Lease" color="#f59e0b" value={adminData.devices?.leased ?? 0} subtitle="Currently assigned" />
          <StatCard icon={<IconTool size={17} />} title="Maintenance" color="#ef4444" value={adminData.devices?.maintenance ?? 0} subtitle="Being serviced" />
        </div>
      )}
      ...
      {isFinance && adminData && (
        <div className="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-4 gap-4">
          <StatCard icon={<IconDollarSign size={17} />} title="Monthly Revenue" color="#22c55e" value={formatCurrency(adminData.payments?.monthly ?? 0)} subtitle="This month" />
          <StatCard icon={<IconClock size={17} />} title="Pending" color="#f59e0b" value={formatCurrency(adminData.payments?.pending?.amount ?? 0)} subtitle={`${adminData.payments?.pending?.count ?? 0} invoices`} />
          <StatCard icon={<IconAlertCircle size={17} />} title="Overdue" color="#ef4444" value={formatCurrency(adminData.payments?.overdue?.amount ?? 0)} subtitle={`${adminData.payments?.overdue?.count ?? 0} students`} />
          <StatCard icon={<IconTrendingUp size={17} />} title="Annual Revenue" color="#3b5bfc" value={formatCurrency(adminData.payments?.annual ?? 0)} subtitle="Year to date" />
        </div>
      )}
      ...
      <div className="grid grid-cols-1 xl:grid-cols-3 gap-4">
        ...
        <div className="card p-5">
          ...
          <div className="space-y-3.5">
            {(isStudent ? [
              { icon: <IconCheckCircle size={13} />, color: "#22c55e", text: "Your lease was approved",       time: "2 days ago" },
              { icon: <IconLaptop size={13} />,      color: "#3b5bfc", text: "Device picked up from Center A", time: "3 days ago" },
              { icon: <IconClipboard size={13} />,   color: "#f59e0b", text: "Application submitted",          time: "5 days ago" },
            ] : [
              { icon: <IconClipboard size={13} />,   color: "#f59e0b", text: "New application from M. Santos", time: "10 min ago" },
              { icon: <IconCheckCircle size={13} />, color: "#22c55e", text: "Lease #L-1042 approved",         time: "1 hr ago" },
              { icon: <IconLaptop size={13} />,      color: "#3b5bfc", text: "Device returned: Dell XPS 15",   time: "2 hrs ago" },
              { icon: <IconCreditCard size={13} />,  color: "#a78bfa", text: "Payment received from J. Reyes", time: "3 hrs ago" },
              { icon: <IconTool size={13} />,        color: "#ef4444", text: "Maintenance flagged: ThinkPad",  time: "Yesterday" },
          ...
          <div className="card p-5">
            ...
            <div className="space-y-3.5">
              {(["active","pending","returned","rejected"] as const).map(s => (
                <MiniBar key={s} label={s.charAt(0).toUpperCase() + s.slice(1)}
            ...
            return (
              <div>
                <div className="flex justify-between text-xs mb-1.5" style={{ color: "var(--text-muted)" }}>
                  <span>Lease progress</span><span className="font-semibold">{pct}%</span>
                </div>
